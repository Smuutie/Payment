...
public class TransactionController {
    ...
    public ResponseEntity<Page<TransactionGetDTO>> getTransactions(@PageableDefault(size = 100) Pageable pageable){
        return ResponseEntity.ok(transactionService.getTransactions(pageable));
    }

    @GetMapping(path = "/{transactionId}", produces = MediaType.APPLICATION_JSON_VALUE, consumes = {MediaType.APPLICATION_JSON_VALUE, MediaType.APPLICATION_XML_VALUE})
    public ResponseEntity<TransactionGetDTO> getTransaction(@PathVariable("transactionId") UUID merchantId){
    ...
    @DeleteMapping(path = "/{transactionId}", consumes = {MediaType.APPLICATION_JSON_VALUE, MediaType.APPLICATION_XML_VALUE})
    public ResponseEntity<Void> deleteTransaction(@PathVariable("transactionId") UUID transactionId){
        return transactionService.deleteTransaction(transactionId) ? ResponseEntity.ok().build() : ResponseEntity.badRequest().build();
    }
